...
```javascript
- 输出脱敏：日志/告警/外显字段做脱敏；敏感字段移除（Set 删除 `password`、`token`）。
...
function validate(obj, schema) {
  for (const f of (schema.required || [])) {
    if (obj[f] === undefined || obj[f] === null) throw new Error(`missing ${f}`);
  }
  if (schema.properties?.email?.pattern) {
    const re = new RegExp(schema.properties.email.pattern);
    if (!re.test(String(obj.email || ''))) throw new Error('invalid email');
  }
  if (schema.properties?.status?.type === 'number') {
    if (obj.status !== undefined && typeof obj.status !== 'number') throw new Error('status must be number');
  }
}
validate($json, schema);
return items;
```javascript
- 提示：生产可接入更完善的校验库，但在 n8n Function 中保持轻量即可。

### 测试与环境隔离
- 区分开发/预发/生产环境：凭据与 `WEBHOOK_URL` 绑定到对应域名。
- 端到端测试：使用 `httpbin`/Mock 服务做集成测试；失败路径同样覆盖。
- 变更策略：变更先在预发验证，通过后再推广至生产。
...
N8N_ENCRYPTION_KEY=please_replace_with_random_key
N8N_LOG_LEVEL=info
...
```bash
- 建议：
  - 不在工作流里硬编码凭据，统一用 Credentials 管理；按环境切分。
  - `N8N_ENCRYPTION_KEY` 必填且保密；不同环境使用不同 key。
  - `N8N_WEBHOOK_URL` 指向外网可达域名，反向代理做 TLS 与鉴权。
  - 生产开启审计与备份：周期导出工作流与凭据快照（不含明文）。

### 故障演练：限速与超时
- 目的：验证工作流在外部 API 限速/超时场景下的健壮性。
- 演练一（超时）
  - 将 `HTTP Request` 指向：`https://httpbin.org/delay/10`（延迟 10s）。
  - 配置超时：在节点 `Options` 中设置 `Timeout (ms)=3000`。
  - 观察：请求会超时失败；打开 `Continue On Fail`，分支进入告警路径。
  - 兜底：在失败分支写入队列/日志，后续人工或定时重试。
- 演练二（限速）
  - 模拟高频调用：批量触发 `HTTP Request`（或使用批次节点）。
  - 加入节流：在分支中插入 `Wait`（如 500ms）与 IF 判断重试次数（`$flow.retryCount`）。
  - 退避策略：指数退避（1s → 2s → 5s），上限后写入死信队列。
- 观测与记录
  - 提高日志级别到 `debug`（临时），观察失败原因与状态码。
  - 将演练摘要通过 Webhook/IM 推送至监控渠道。
 
 ### 重试与退避模板
 - 结构：`HTTP Request → IF(失败) → Wait(退避) → Function(计数) → Merge → 重试 → 超限写死信队列`。
 - 关键设置：在 `HTTP Request` 打开 `Continue On Fail=true`；IF 判断表达式示例：`{{ ($json.status || 0) >= 500 }}`。
 - 计数逻辑（Function 节点）：
 ```javascript
 // Function: retry counter
 const count = $flow.get('retryCount') || 0;
 $flow.set('retryCount', count + 1);
 return items;

 - 退避时间（Wait 节点表达式）：`{{ Math.min(1000 * Math.pow(2, ($flow.get('retryCount') || 0)), 10000) }}`（最大 10s）。
 - 超限处理：IF `{{$flow.get('retryCount') >= 5}}` → 写入死信队列（DB/消息队列）并通知 IM。
 
 ### 观测与告警
 - 指标建议：成功率、错误率、平均/95% 延迟、队列长度、重试次数分布、死信队列堆积。
 - 日志采样：临时将日志级别提高到 `debug`（仅在预发），关键节点打点 `requestId`、`status`、`durationMs`。
 - 告警路由：Slack/Telegram/钉钉、邮件、Webhook；按严重程度分层（P1/P2）。
 - 事件上报示例（表达式构建）：`{"event":"workflow_failed","node":"HTTP Request","status":{{$json.status || 0}},"requestId":"{{$flow.requestId}}","retryCount":{{$flow.get('retryCount') || 0}}}`。
 

 ## 实战模板

### 使用建议
- 标准化字段（Set）与入参校验（Function），维持稳定 JSON 契约。
- 可选增强：引入“重试与退避模板”与“观测与告警”，在失败分支写队列并通知。
- 对关键写入开启 `Continue On Fail`，使用 IF + Wait 做指数退避与上限控制。

### 模板一：API 拉取与落库
- 触发：`Schedule(Cron)` 每日 08:00。
- 拉取：`HTTP Request` 访问第三方 API，分页循环。
- 清洗：`Function` 标准化字段与类型。
- 落库：`MySQL/Postgres` 使用 Upsert 写入。
- 通知：`Slack/邮件` 推送统计摘要。
- 注意：设置超时/重试，分页边界与幂等。

### 模板二：表单提交流转（前端 → 审批 → 存档）
- 入口：`Webhook` 接收表单 JSON，校验签名与必填。
- 审批：`IF/Switch` 根据规则路由，等待审批结果。
- 存档：`DB` 写入审批记录；失败走旁路队列。
- 反馈：`Respond to Webhook` 返回处理结果给前端。
- 告警：异常路径写 IM 通知与日志。

### 模板三：CSV 导入与清洗
- 输入：`Webhook/Binary` 接收文件；`Spreadsheet File` 转为 JSON。
- 清洗：`Function/Item Lists` 过滤与映射。
- 分批：`Split in Batches` 控制批量入库速率。
- 入库：`DB` 写入；失败 items 入死信队列。
- 汇总：`Set` 统计条数与错误，`Notify` 汇报。
 
 ## 常见问题（FAQ）

### 节点没有输出数据？
- 检查输入：在节点的 `Input` 面板查看 `items` 是否为空；上游是否执行成功。
- 检查表达式：确认使用了正确作用域（`$json` vs `$node["..."]`），避免引用空值。
- 执行模式：调试时确保点击了“执行工作流”；触发器是否处于激活状态。
- 连线与分支：是否遗漏分支连线、IF 条件是否全部走向空分支。

### Webhook 收不到请求？
- 地址与方法：确认路径 `http://<host>/webhook/<path>` 与方法匹配（GET/POST）。
- 反代与 `WEBHOOK_URL`：在代理/公网场景设置 `WEBHOOK_URL` 为外部可达地址。
- 端口与防火墙：容器端口映射 `5678:5678` 是否存在；防火墙是否放行。
- 快速自测：
```bash
curl -i -X POST http://localhost:5678/webhook/demo \
  -H 'Content-Type: application/json' -d '{"ping":1}'
```python

### OAuth2 无法授权？
- 回调地址：确保与部署域名一致（含协议与端口），已在第三方平台白名单中。
- Scope 权限：避免过宽或缺少必需权限；查看错误提示与日志。
- 令牌管理：处理刷新令牌与过期，失败分支写入告警。

### 数据库连接失败？
- 网络与认证：容器网络可达、主机/端口正确、用户名密码有效。
- SSL/证书：需要安全连接时配置证书；排查连接参数。
- SQL 报错：检查字段类型与空值，避免拼接表达式导致语法错误。

### 表达式报错或取值为 `undefined`？
- 作用域：区分 `{{$json}}`（当前输入）与 `{{$node["Name"].json}}`（指定节点输出）。
- 空值防御：`{{$json.foo && $json.foo.bar || 0}}`，避免属性未定义。
- 转义：在 JSON 字段中使用表达式时注意引号与转义规则。

### 执行卡住或很慢？
- 外部 API 限速：加入 `Wait` 与重试策略；拆分大批量任务。
- 并发争用：避免对同一资源并发写；必要时加锁或串行执行。
- 监控日志：提高 `N8N_LOG_LEVEL` 到 `debug` 进行短期定位。

### 升级后凭据丢失？
- 加密密钥：`N8N_ENCRYPTION_KEY` 必须保持不变；否则无法解密历史凭据。
- 备份策略：升级前后备份 `n8n_data`（以及外部数据库）。

## 参考资源

- 官方文档与指南：
  - 文档主页：https://docs.n8n.io/
  - 基础指南（Workflows/Nodes/Expressions）：https://docs.n8n.io/getting-started/
  - 节点目录（按服务/协议）：https://docs.n8n.io/integrations/
- 代码与版本：
  - GitHub 仓库：https://github.com/n8n-io/n8n
  - Releases（升级参考）：https://github.com/n8n-io/n8n/releases
  - Issue Tracker（问题排查）：https://github.com/n8n-io/n8n/issues
- 社区与生态：
  - 官方论坛：https://community.n8n.io/
  - 示例与模板库：https://n8n.io/workflows/
  - 优秀资源集合（Awesome n8n）：https://github.com/ditrit/awesome-n8n
- 学习资料：
  - 视频教程与直播回放（YouTube/社区）
  - 博客与实践文章（按场景：ETL、通知、API 编排）
- 运维与部署：
  - Docker/Compose 部署指南：https://docs.n8n.io/hosting/
  - 环境变量与配置项：https://docs.n8n.io/hosting/environment-variables/

## 差异对比：n8n / Dify / Coze

- 定位与核心能力：
  - n8n：通用工作流自动化与集成，节点编排与数据流处理
  - Dify：LLM 应用开发与知识库/RAG/工作流编排，AI 原生能力
  - Coze：低门槛 Agent/Bot 搭建与部署，平台生态内集成
- 使用门槛与扩展：
  - n8n：节点丰富、表达式灵活；需理解 items/作用域/错误分支
  - Dify：模型/知识库/工作流概念较多；需要 AI 应用基础
  - Coze：上手快；平台内集成强；自托管与跨平台扩展受限
- 典型场景：
  - n8n：跨系统数据同步、API 编排、通知告警、轻量 ETL
  - Dify：RAG 问答、智能助手、企业内部 AI 应用与工作流
  - Coze：对话型机器人、社交平台/企业内部轻应用
- 部署与运维：
  - n8n：自托管简单（Docker/Compose），持久化与备份清晰
  - Dify：多组件（API/Web/DB/向量库），资源要求更高
  - Coze：平台托管为主，企业内自托管能力受限

## 总结
- 学习路径清晰：先 Docker 安装，再理解核心概念（工作流、节点、表达式、凭据），随后用示例快速上手。
- 实战导向：围绕常用触发器与节点，提供数据库与前端对接的流程模板。
- 生产落地：给出凭据安全、内网部署、日志监控、升级备份与扩展策略建议。
- 对比视角：了解 n8n、Dify、Coze 的定位与生态，选择适合的落地方案。

## 附录：工作流导出与导入

- 导出工作流：
  - 在工作流编辑页面，点击“导出”或“下载 JSON”，保存到本地。
- 导入工作流：
  - 在主页点击“新建”→“导入 from file/JSON”，选择之前导出的文件。
- 示例片段（最小可运行 Webhook → Set）：
```json
{
  "nodes": [
    {
    ...
    },
    {
      "parameters": {
        "values": {
          "string": [
            {"name": "message", "value": "{{$json.name || 'n8n'}}"}
          ]
        }
      },
      "id": "Set",
...
```javascript
- 使用方式：导入后在编辑器中补充下游节点（如 `HTTP Request`/`Respond to Webhook`）。

### 示例：Quickstart 工作流（HTTP Trigger → Function → HTTP Request）
- 说明：此示例与“快速上手”一致；导入后可直接触发并调试。版本差异可能导致字段名略有不同，请以实际 UI 为准。
```json
{
  "nodes": [
    {
    ...
    {
      "parameters": {
        "functionCode": "return items.map(item => { const data = item.json; return { json: { message: `Hello, ${data.name || 'n8n'}!`, ts: Date.now() } }; });"
      },
      "id": "Function",
      "name": "Function",
  ...
  "connections": {
    "HTTP Trigger": { "main": [[{ "node": "Function", "type": "main", "index": 0 }]] },
    "Function": { "main": [[{ "node": "HTTP Request", "type": "main", "index": 0 }]] }
  }
}
